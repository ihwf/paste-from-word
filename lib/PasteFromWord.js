...
class PasteFromWord {
    constructor(config) {
        this.imageHandler = config && config.imageHandler
        this.supportedImageTypes = ['image/png', 'image/jpeg', 'image/gif']
    }
    /**
     * 运行解析器
    ...
    parse(pasteEvent, next) {
        this.e = pasteEvent
        const clipboardData = (this.clipboardData = pasteEvent.clipboardData)
     ...
     */
    checkPaster(paster) {
        const mswordHtml = this.getClipboardData(paster, 'html')
     ...
     */
    parser(paster, next) {
        const mswordHtml = this.stripHtml(this.getClipboardData(paster, 'html'))
        const plain = this.getClipboardData(paster, 'plain')
        const dataTransferRtf = this.getClipboardData(paster, 'rtf')
        const pfwEvtData = {
            html: mswordHtml,
            text: plain,
            rtf: dataTransferRtf,
    ...
    imageHandlerWrapper(img) {
        const self = this
        return new Promise(function (resolve) {
            self.imageHandler(self.createBlobWithImageInfo(img), resolve)
        })
    }
}
